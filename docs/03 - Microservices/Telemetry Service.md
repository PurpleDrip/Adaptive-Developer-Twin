...
- `BatchProcessor` uses `datetime.utcnow()` which is deprecated in Python 3.12+ — tz-naive datetimes can cause subtle bugs when compared with tz-aware MongoDB datetimes
- `BatchProcessor` reschedules on config change — only testable with a running scheduler (integration-only)
